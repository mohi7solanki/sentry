...
def test_record_http_metrics(incr):
    responses.add(responses.GET, 'http://example.com')
    response = http.safe_urlopen('http://example.com')

    http.record_http_metrics(
        response,
        key='foo',
...
@responses.activate
@patch('sentry.utils.metrics.incr')
def test_record_http_metrics_defaults(incr):
    responses.add(responses.GET, 'http://example.com')
    response = http.safe_urlopen('http://example.com')

    http.record_http_metrics(response)
...
@responses.activate
@patch('sentry.utils.metrics.incr')
def test_record_http_metrics_on_failure(incr):
